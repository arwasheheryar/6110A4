...
This study reanalyses publicly available single-cell RNA-seq data from Bai et al. (2024), which profiled the nasal mucosa of mice at multiple timepoints following IAV infection restricted to the nasal cavity. A pre-built Seurat object containing count data and metadata was provided by the course instructor. The primary biological question addressed here is how macrophages in the nasal mucosa respond transcriptionally at peak viral load (day 5 post-infection; D05) compared to uninfected controls (Naive), and which innate immune pathways are preferentially activated during this window of maximal viral replication. Seurat v5 was used for all single-cell processing and clustering, DESeq2 for pseudobulk differential expression analysis, and clusterProfiler for over-representation analysis (ORA), following best practices for scRNA-seq analysis outlined by Heumos et al. (2023).
...
Pseudobulk DESeq2 analysis comparing macrophages at D05 (peak viral load) to Naive macrophages identified differentially expressed genes at p_val_adj < 0.05. The top 15 upregulated genes are shown in Table 1, including *Zbp1* (avg_log2FC = 1.54), *Gm42418* (avg_log2FC = 1.82), *Rsad2* (avg_log2FC = 1.42), *Stmn1* (avg_log2FC = 1.59), *Ifih1* (avg_log2FC = 1.08), and *Xaf1* (avg_log2FC = 1.17) — a coherent set of interferon-stimulated genes (ISGs) and innate antiviral sensors. Violin plots of the top four upregulated genes confirmed higher expression at D05 relative to Naive within the macrophage population, with *Gm42418* and *Isg15* showing the most pronounced differences (Figure 8).
...
Taken together, this analysis demonstrates that nasal mucosal macrophages are transcriptionally active innate immune effectors at the site of IAV replication, deploying a coordinated ISG programme centered on IFN signaling, viral RNA sensing via multiple PRR pathways, and direct antiviral effector activity at peak viral load. These findings are consistent with the established role of type I IFN and its downstream effectors in controlling early IAV replication and highlight the nasal mucosa as an active immunological interface rather than a passive conduit for viral infection.

---

## References

Bai, Y., Huang, C., Hu, X., et al. (2024). Single-cell transcriptomic analysis of the nasal mucosa response to influenza A virus infection. *Nature Communications*, 15, 6892. https://doi.org/10.1038/s41467-024-51292-4
